<template>
    <div class="col-start-3 row-start-1 flex flex-col gap-5 text-2xl">
        <div v-for="i in [1, 2, 3]" :key="i" class="bg-slate-500 border-2 border-solid border-black overflow-hidden"
            :class="{ 'border-green-50': store.selectedPlayer() == i-1, 'bg-slate-300 text-slate-900': store.isActive() }">
