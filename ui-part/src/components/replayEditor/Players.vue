...
div(class="col-span-3 row-start-2 row-span-1 lg:col-span-1 lg:row-start-2 flex flex-col gap-5 text-2xl")
  div(v-for="i in [1, 2, 3]" :key="i"
    ...
    :class="{ 'border-gray-300 font-bold font-lekton-bold  text-slate-900': store.selectedPlayer() == i - 1 && store.isActive(), 'bg-slate-300': store.isActive() }")
    div(class="rounded mx-5 text-2xl lg:text-3xl") {{ i }}. {{ name(i - 1) }}
